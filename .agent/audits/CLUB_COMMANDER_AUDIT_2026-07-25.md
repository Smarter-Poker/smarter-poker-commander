...
Both fixes are merged to `commander-shared` `main` as **v0.1.2**, and **v0.1.2 is published and live in production**. They were deliberately *not* forked into the Commander repo as local overrides: doing so would load a second copy of the module, and since `channelManager` is a module-level singleton, every venue would end up with two Supabase channels per tab instead of one. Correctness there was worth the release step.

Publishing was the one action the audit environment could not perform directly — its egress proxy blocks every package registry, `npm.pkg.github.com` included. Rather than hand the step off, the release was automated: `.github/workflows/publish.yml` now publishes the package from GitHub Actions using the built-in `GITHUB_TOKEN` with `packages: write`, guarded by a version check so it publishes only when `package.json` carries a version the registry has not seen. Pushing that workflow to `main` triggered its own first run, which published 0.1.2. Commander's dependency was then moved from `^0.1.1` to `^0.1.2` — a deliberately decisive test, since npm cannot resolve a version that does not exist. The build succeeded (`changed 1 package`), and production now serves commit `4d0e7f5f` with the realtime fix live. The workflow remains in place, so future releases need only a version bump.

One note for the next release: this repo's manifest read `0.1.0` while the published artifact was `0.1.1`, and the published copy carried `publishConfig.access: "public"` where the repo says `"restricted"`. The `src/` trees were byte-identical, so only the manifest had drifted; `access` was left as the repo had it rather than silently changing publish visibility.
...
## Hardening phase (2026-07-27) and a dependency-delivery change

The audit changed 161 files with nothing guarding them, and `npm test` turned out to be theatre: vitest was wired to `npm test` while the repository contained no unit tests at all, and the CI step carried `continue-on-error: true`, so it could neither fail nor report. Worse, with no config vitest would have collected `tests/e2e/pin-gate.spec.ts` and died on its Playwright import. There is now a `vitest.config.js` scoping unit runs to `tests/unit/**`, a 36-assertion suite over the three highest-risk fixes — ICM payout math, rate-limit identity and staff-session forgery — and the CI Test step is a real gate. Lint remains advisory; that is pre-existing debt and blocking on it would stall unrelated work.

**Read this before touching the shared package again.** Commander no longer installs `@smarter-poker/commander-shared` from the registry. Vercel preview builds were failing with `401 Unauthorized` against `npm.pkg.github.com` because the `NPM_TOKEN` environment variable is scoped to Production only, so previews had no credential. The fix was to vendor the package: `package.json` now points at `file:vendor/commander-shared`, the scoped-registry and auth lines in `.npmrc` are commented out, and the package source is committed under `vendor/`. Preview builds install locally and never contact the registry.

The consequence is a drift hazard that will not announce itself. The vendored tree is a **snapshot**, verified at merge time as v0.1.2 — `useCommanderSync.js` at `605f5480` and `FloorCallAlert.js` at `022fd931`, both matching the published build byte for byte. But publishing a new version of the shared package will no longer reach Commander at all; someone must re-copy `vendor/commander-shared` deliberately. Meanwhile World Hub still resolves the package from the registry, so the two consumers now receive shared code by different mechanisms and can silently diverge. The publish workflow added at `.github/workflows/publish.yml` still works and is still worth using so the registry stays authoritative — but it is no longer sufficient on its own for Commander.
