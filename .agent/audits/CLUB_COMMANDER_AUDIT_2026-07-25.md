...
**FloorCallAlert — genuinely blocked, not deferred by choice.** This one does require a package release. `CommanderLayout.jsx` imports it as `'./FloorCallAlert'` — a relative specifier *inside* the package — so a local override in this repo cannot intercept it, and CommanderLayout wraps every staff page. The component's header advertises a 5-second polling fallback that was never implemented, so if the Supabase realtime channel drops on a long-lived floor tablet, new floor calls stop alerting until someone reloads. Closing it needs a `@smarter-poker/commander-shared` publish. Note also that the shared repo's `main` reads version `0.1.0` while both consumers pin `^0.1.1`, so the registry artifact is ahead of the repository; that discrepancy should be reconciled before publishing.
